...
/**
 * GOTO Engine — Browser Module Build Script
 *
 * 构建 dist/browser/ 为自包含目录：
 *   1. 复制 goto-engine.js 和 goto-engine-component.js → dist/browser/
 *   2. 生成自包含的 dist/browser/index.mjs
 *   3. 生成 SHA256
 *
 * 注意：Engine 算法不被修改，仅复制和包装。
 */
...
const DIST_DIR = path.join(ROOT, 'dist', 'browser');
...
function main() {
  ...
  // 1. 复制 Engine 源文件
  const engineSrc = path.join(ROOT, 'goto-engine.js');
  const componentSrc = path.join(ROOT, 'goto-engine-component.js');
  
  if (fs.existsSync(engineSrc)) {
    copyFile(engineSrc, path.join(DIST_DIR, 'goto-engine.js'));
    console.log('Copied goto-engine.js → dist/browser/goto-engine.js');
  }
  if (fs.existsSync(componentSrc)) {
    copyFile(componentSrc, path.join(DIST_DIR, 'goto-engine-component.js'));
    console.log('Copied goto-engine-component.js → dist/browser/goto-engine-component.js');
  }

  // 2. 生成自包含的 index.mjs
  const indexMjs = `'use strict';
...
`;
  ...
  console.log('Generated dist/browser/index.mjs (self-contained)');

  const entryFile = path.join(DIST_DIR, 'index.mjs');
  const hash = sha256(entryFile);
  const size = fs.statSync(entryFile).size;

  console.log('\n=== GOTO Engine Browser Module Build ===');
  console.log('Entry: dist/browser/index.mjs');
