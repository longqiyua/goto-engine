...
>
> **本目录为 GOTO Engine 的 Javascript 实现（模拟智能母体）**。算法运行时（`goto-engine.js`）+ 独立组件 API（`goto-engine-component.js`）+ 插件宿主（`plugin-api.js`）+ 反馈通道（`feedback.js`）+ 可选语义扩展（`semantic/`）五位一体。运行于浏览器 / WebView / Node.js，无外部依赖，所有搜索、索引、上下文、反馈默认全部在本地完成。包含完整 18 模块，是 JS/Rust 双版本对齐的算法母体。License：GNU AGPL-3.0（详见 [LICENSE](LICENSE)）。
