...
>
> **本目录为 GOTO Engine 的 Kotlin 实现（Android 应用搜索引擎）**。聚焦 Android 原生应用搜索匹配场景：`AppSearchEngine` v5 实现 18 层匹配维度 + 并行协程；`FuzzyMatchEngine` v1.1 实现 QWERTY/T9 双布局键盘感知编辑距离；`AdaptiveRefresh` 提供 Kotlin 独有的可丢弃搜索调度（防抖+节流+自适应延迟）；SQLite + SharedPreferences 双轨存储。提供与 JS/Rust 对齐的组件层（`EngineComponent` + `QueryResponse` + 事件系统 + 错误码），通过接口契约保证三版本行为一致。与 JS/Rust 版本互补——JS/Rust 是模拟智能母体（完整 18 模块），Kotlin 聚焦应用搜索匹配（5 核心 + 数据/存储层）。License：GNU AGPL-3.0（详见 [LICENSE](LICENSE)）。
